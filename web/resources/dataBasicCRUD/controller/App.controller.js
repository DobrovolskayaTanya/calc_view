...
sap.ui.define([
	...
	return BaseController.extend("opensap.odataBasic.controller.App", {
		...
		onBatchDialogPress: function () {
			...
			view._bDialog = sap.ui.xmlfragment("opensap.odataBasic.view.batchDialog", this // associate controller with the fragment  
			);
		...
		onSubmitBatch: function () {
			...
			var batchModel = new sap.ui.model.odata.v2.ODataModel("/xsodata/user.xsodata/", oParams); //var batchChanges = [];    var mParams = {};    mParams.groupId = "1001";    mParams.success = function() {     sap.m.MessageToast.show("Create successful");    };    mParams.error = this.onErrorCall; 

			for (var k = 0; k < newUserList.length; k++) {
				batchModel.create("/Users", newUserList[k], oParams);
			}
